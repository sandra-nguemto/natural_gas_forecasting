...
# NEW: Texas historical + forecast helper
@st.cache_data
def get_texas_hist_and_forecast():
    """
    Return monthly historical + forecast series for Texas.
    Uses gas_long (historical) and gas_forecast (forecast).
    """
    con = duckdb.connect(DB_PATH, read_only=True)
    query = """
        WITH hist AS (
...
st.caption(
    "Monthly natural gas delivered to consumers by state (MMcf). "
    "Historical data with an optional Texas 12-month forecast panel."
)
...
with st.sidebar:
    ...
    st.markdown("---")
    # NEW: forecast toggle
    show_texas_forecast = st.checkbox(
        "Show Texas 12-month forecast panel",
        value=True
    )
...
# ---------- TEXAS HISTORICAL + FORECAST PANEL ----------
st.markdown("---")
st.subheader("Texas: Historical + 12-Month Forecast (Monthly)")

if show_texas_forecast:
    df_tx = get_texas_hist_and_forecast()

    if df_tx.empty:
        st.info("No forecast data found for Texas. Make sure gas_forecast is populated.")
    else:
        fig_tx = px.line(
            df_tx,
            x="Date",
            ...
            title="Texas Natural Gas Demand (Historical vs 12-Month Forecast)"
        )

        # Add vertical line at forecast start using a shape (more robust than add_vline)
        if "forecast" in df_tx["kind"].values:
            forecast_start = df_tx.loc[df_tx["kind"] == "forecast", "Date"].min()
            forecast_start = pd.to_datetime(forecast_start)

            y_min = df_tx["value"].min()
            y_max = df_tx["value"].max()
    ...
    st.info("Enable the checkbox in the sidebar to display the Texas 12-month forecast panel.")